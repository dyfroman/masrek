...
CHECKS: dict[str, Check] = {
    ...
    "A02": Check(
        ...
        sast_tools=["semgrep", "gitleaks"],
        detectability="full",
        sast_detectability="partial",
        combined_detectability="full",
        ...
        sast_reason="semgrep detects misconfigurations in code + gitleaks finds exposed secrets in config files. Runtime/server config needs DAST.",
        sast_reason_he="semgrep מזהה תצורה שגויה בקוד + gitleaks מוצא סודות חשופים בקבצי תצורה. תצורת שרת/ריצה דורשת DAST.",
    ),
    "A03": Check(
        id="A03",
        ...
        sast_reason="osv-scanner + trivy check lockfiles and dependencies against vulnerability databases for known CVEs.",
        sast_reason_he="osv-scanner + trivy בודקים קבצי נעילה ותלויות מול מסדי נתוני פגיעויות לזיהוי CVE ידועים.",
    ),
    "A04": Check(
        id="A04",
    ...
    "A05": Check(
        ...
        sast_reason="semgrep detects injection patterns in source code (SQLi, XSS, command injection, SSTI).",
        sast_reason_he="semgrep מזהה דפוסי הזרקה בקוד מקור (SQLi, XSS, הזרקת פקודות, SSTI).",
    ),
    "A06": Check(
        id="A06",
        ...
        sast_reason="semgrep flags known insecure-design code patterns only (eval, unsafe deserialization, mass assignment). Architectural design flaws need human review.",
        sast_reason_he="semgrep מזהה דפוסי עיצוב לא מאובטח בקוד בלבד (eval, deserialization לא מאובטח). פגמי עיצוב ארכיטקטוניים דורשים סקירה ידנית.",
    ),
    "A07": Check(
        id="A07",
        ...
        sast_reason="semgrep + gitleaks detect hardcoded credentials, API keys, and tokens in source code. Auth logic flaws need DAST + manual review.",
        sast_reason_he="semgrep + gitleaks מזהים סיסמאות, מפתחות API וטוקנים מוטמעים בקוד מקור. פגמי לוגיקת אימות דורשים DAST + סקירה ידנית.",
    ),
    "A08": Check(
        id="A08",
        ...
        sast_reason="trivy covers dependency integrity and IaC/CI misconfigurations. Build-system/CI-pipeline tampering and SRI require human review.",
        sast_reason_he="trivy מכסה שלמות תלויות ותצורת IaC/CI שגויה. זיוף מערכת הבנייה/צינור CI ו-SRI דורשים סקירה ידנית.",
    ),
    "A09": Check(
        id="A09",
        ...
        sast_reason="semgrep detects missing logging patterns and sensitive data in logs. Full logging coverage needs runtime access + SIEM review.",
        sast_reason_he="semgrep מזהה דפוסי רישום חסרים ומידע רגיש ביומנים. כיסוי רישום מלא דורש גישת ריצה + סקירת SIEM.",
    ),
    "A10": Check(
        id="A10",
